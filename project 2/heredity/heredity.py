...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    """   
    probabilities = []
    allPersons = people.keys()
    zero_genes = []
    for person in allPersons:
        if (person not in one_gene) and (person not in two_genes):
            zero_genes.append(person)
    ...
    for person in have_trait:
        ...
        probabilities.append(PROBS["trait"][genesCount][True])
    ...
    for person in allPersons:
        if person not in have_trait:
            ...
            probabilities.append(PROBS["trait"][genesCount][False])  
...
def normalize(probabilities):
    ...
    for person in probabilities:
        ...
        factorT = 1 / (sum(trait.values()))
        for x in trait:
            trait[x] *= factorT
            
        gene = probabilities[person]["gene"]
        
        factorG = 1 / (sum(gene.values()))
        for y in gene: 
            gene[y] *= factorG
